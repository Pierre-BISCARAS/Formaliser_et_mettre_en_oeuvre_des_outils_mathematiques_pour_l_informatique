...
def rectanglecCollision(c1, c2):
    if (c1[0] < c2[0] + constants.PERSON_RADIUS and 
    c1[0]+ constants.PERSON_RADIUS > c2[0] and 
    c1[1] < c2[1] + constants.PERSON_RADIUS and 
    c1[1]+ constants.PERSON_RADIUS > c2[1]):
        return True
    return False
...
def circleCollision(c1, c2):
    ...
    '''
    distance= math.sqrt((c2[0]-c1[0])**2+(c2[1]-c1[1])**2)-constants.PERSON_RADIUS
    if distance <= 0 :
        return True
    return False
...
def computeCollisions(persons, personNumber):
    ...
    for j in range (len(persons)):
        ...
        if ( j != personNumber):
            ...
            if rectanglecCollision((p[0],p[1]),(q[0],q[1])):
